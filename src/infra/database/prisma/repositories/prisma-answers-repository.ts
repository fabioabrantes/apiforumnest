...
export class PrismaAnswersRepository implements IAnswersRepository {
  ...
  async findById(id: string): Promise<Answer | null> {
    ...
    return PrismaAnswerMapper.convertToAnswerDomain(answer);
  }
  ...
  async findManyByQuestionId(questionId: string, { page }: PaginationParams,): Promise<Answer[]> {
    ...
    return answers.map(PrismaAnswerMapper.convertToAnswerDomain);
  }

  async create(answer: Answer): Promise<void> {
    const data = PrismaAnswerMapper.convertToAnswerPrisma(answer);
